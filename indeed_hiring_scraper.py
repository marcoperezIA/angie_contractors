...
en Indeed, Glassdoor y LinkedIn - Solo LA y Bay Area.
...
SKIP_DOMAINS = (
    "duckduckgo", "google", "indeed", "glassdoor", "linkedin",
    "facebook", "yelp", "bbb.org", "twitter", "instagram",
    "youtube", "wikipedia", "bloomberg", "crunchbase", "ziprecruiter",
    "salary.com", "comparably", "builtin", "ambitionbox", "tiktok",
    "pinterest", "reddit", "amazon", "apple.com/app",
)
...
def scrape_indeed(job_page, contact_page, query, location, saved_urls, company_cache, max_pages=5):
    results = []
    ...
    for start in range(0, max_pages * 10, 10):
        ...
        for card in job_cards:
            try:
                title_el = card.find(["h2", "h3"]) or card.find("a", class_=re.compile(r"title|jcs-JobTitle"))
                title = title_el.get_text(strip=True) if title_el else ""

                company_el = card.find(attrs={"data-testid": "company-name"}) or card.find("span", class_=re.compile(r"company|companyName"))
                ...
                sal_el = card.find(class_=re.compile(r"salary|estimated-salary|salaryText"))
...
def scrape_glassdoor(job_page, contact_page, query, location, saved_urls, company_cache, max_pages=3):
    results = []
...
def scrape_linkedin(job_page, contact_page, query, location, saved_urls, company_cache, max_pages=3):
    ...
    for pg in range(max_pages):
        ...
        for card in job_cards:
            try:
                ...
                company_el = card.find(class_=re.compile(r"base-search-card__subtitle|job-card-container__company"))
                company = company_el.get_text(strip=True) if company_el else ""

                loc_el = card.find(class_=re.compile(r"job-search-card__location|job-card-container__metadata"))
                loc = loc_el.get_text(strip=True) if loc_el else ""
...
def create_browser(p):
    ...
    # Dos pestañas: una para jobs, otra para buscar contactos
    ...
    with sync_playwright() as p:
        ...
        for query in SEARCH_QUERIES:
            for location in LOCATIONS:
                ...
                print(f"\n  [{query}] en [{city}]")
                try:
                    results = scrape_glassdoor(job_page, contact_page, query, city, saved_urls, company_cache)
                    total += len(results)
        ...
        for query in SEARCH_QUERIES:
            for location in LOCATIONS:
                print(f"\n  [{query}] en [{location}]")
                try:
                    results = scrape_indeed(job_page, contact_page, query, location, saved_urls, company_cache)
                    total += len(results)
                    print(f"  -> {len(results)} nuevos")
                except Exception as e:
                    print(f"  ERR: {e}")
        ...
        for query in SEARCH_QUERIES:
            for location in LOCATIONS:
                print(f"\n  [{query}] en [{location}]")
                try:
                    results = scrape_linkedin(job_page, contact_page, query, location, saved_urls, company_cache)
                    total += len(results)
                    print(f"  -> {len(results)} nuevos")
                except Exception as e:
                    print(f"  ERR: {e}")
